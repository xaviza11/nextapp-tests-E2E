...
describe('Return random Flight Route', () => {
    test('route has been returned', async () => {
        try {
            ...
            const returnedRoute = await axios.get(host + 'api/flightRoutes/returnRandomRoute', {
                headers: {
                    'Content-Type': 'application/json',
                    'Cookie': `${tokenName}=${token}`
                },
    ...
    test('not return data when route is not public', async () => {
        //warn it's working whit no problems
        expect(true).toBe(false) //! disabled because not delete data of the db 
        /*try {
            await axios.post(host + 'api/auth/signup', {
                email: "test@test.com",
                password: "A1aAAAAAAAAAAAAA",
            ...
            expect(returnedRoute.data).toBe('a')
        } catch (error) {
            expect(error.response.data.message).toBe("No public routes found")
        }*/
    })
})
