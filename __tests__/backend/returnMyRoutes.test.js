...
describe('Return My Routes', () => {
    test('routes has been returned', async () => {
        try {
            ...
            const resultB = await axios.post(host + url, {
                "departure": "LERS",
                "arrival": "LIRQ",
                "departureCity": "REUS",
                "arrivalCity": "PRETOLA",
            ...
            const returnedRoute = await axios.get(host + 'api/flightRoutes/returnMyRoutes', {
                headers: {
                    'Content-Type': 'application/json',
                    'Cookie': `${tokenName}=${token}`
                },
            ...
            expect(returnedRoute.data.routesFound.length).toBe(2)
            expect(typeof returnedRoute.data.routesFound[0]._id).toBe('string')
            expect(typeof returnedRoute.data.routesFound[1]._id).toBe('string')
            expect(typeof returnedRoute.data.routesFound[0].arrival).toBe('string')
            expect(typeof returnedRoute.data.routesFound[1].arrival).toBe('string')
            expect(typeof returnedRoute.data.routesFound[0].departure).toBe('string')
            expect(typeof returnedRoute.data.routesFound[1].departure).toBe('string')
        } catch (error) {
            expect(error).toBe("a")
        }
    })
